...
- (void)query:(NSString*)term{
    ...
    NSArray *arrayPathComponent=[NSArray arrayWithObjects:NSHomeDirectory(),@"Library/Caches",@"eocr.sqlite",nil];
    NSString *databasePath=[NSString pathWithComponents:arrayPathComponent];
  	if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
        ...
        if (level < 0.5)
        {
            sqls = [NSString stringWithFormat:@"select * from data2 where _id MATCH \"%@\"", term ];
            ...
            if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK)
            {
                ...
                while(sqlite3_step(compiledStatement) == SQLITE_ROW)
                {
                    ...
                    aName = [[NSString stringWithUTF8String:(char *)sqlite3_column_text(compiledStatement, 0)] stringByAppendingString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(compiledStatement, 1) ]];
		...
		else
        {
            ...
            if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK)
            {
                ...
                {
                    // Read the data from the result row
                   aName = [[NSString stringWithUTF8String:(char *)sqlite3_column_text(compiledStatement, 0)] stringByAppendingString:[NSString stringWithUTF8String:(char *)sqlite3_column_text(compiledStatement, 1) ]];                    [resultArray addObject:aName];
                }
            ...
            NSArray *wordsAndEmptyStrings = [term componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
            NSArray *words = [wordsAndEmptyStrings filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"length > 0"]];
                //    NSMutableArray * mutableWords = [NSMutableArray new];
                for (NSString * word in words){
                    NSLog(word);
                        NSString * editedWord = [word substringFromIndex:0];
                        //          [mutableWords addObject:editedWord];
                        sqls = [NSString stringWithFormat:@"select * from data2 where _id MATCH \"%@\"", editedWord ];
                        const char *sqlStatement = [sqls UTF8String];
                        sqlite3_stmt *compiledStatement;
                        if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK)
                        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //[cell.textLabel setFrame:CGRectMake(10, 10, 110, 10)];
...
{
    NSString *cellText = [resultArray objectAtIndex:indexPath.row];
    UIFont *cellFont = [UIFont fontWithName:@"ArialMT" size:12.0];
